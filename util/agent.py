...
class Elon():
    def __init__(self, state_dim, action_dim, save_dir, memory_size=100000, checkpoint=None):
        ...
        self.exploration_rate_decay = 0.99999 #0.999995 #0.99999 #0.99999 #0,99975 #999 # 0.9999991 # 0.99999975
        ...
        self.save_every = 5e4   # no. of experiences between saving Neural Net
        self.save_dir = save_dir
        
        self.use_cuda = torch.cuda.is_available()
        
        self.neuralnet = LanderNet2(self.state_dim, self.action_dim).float()
        if self.use_cuda:
    ...
    def save(self):
        save_path = self.save_dir / f"landernet1_{int(self.curr_step // self.save_every)}.chkpt"
        torch.save(
            dict(
        ...
        print(f"landernet saved to {save_path} at step {self.curr_step}")
        pass
