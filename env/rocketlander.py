...
if EASY_MISSIONS:
    START_HEIGHT = 200.0
    START_SPEED = 33.0
    INITIAL_RANDOM = -1.0 #-0.7 #-1.0 #-0.5
...
class RocketLander(gym.Env):
    ...
    def step(self, action):
        ...
        if self.continuous:
            ...
            if (EASY_MISSIONS and (self.legs[0].ground_contact or self.legs[1].ground_contact)) or self.had_ground_contact:
                self.throttle = -1.
                action = 6
                self.had_ground_contact = True
        ...
        largetheta = abs(angle) > THETA_LIMIT
        ...
        if not(CUSTOM_REWARDS):
            ...
            reward = -fuelcost
            if outside or brokenleg or largetheta:
                self.game_over = True            
            ...
            # states are different for this environment
            reward = 0
            
            shaping = - 200 * distance \
                      - 100 * speed \
                      - 1000 * abs(angle) - 30 * abs(vel_a) \
                      + 20 * self.legs[0].ground_contact + 20 * self.legs[1].ground_contact
            ...
            reward += -self.power* 0.3
            reward += -abs(self.force_dir) * 0.3
